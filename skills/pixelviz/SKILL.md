...
- **PR:** keep the deck and captures in an ignored scratch folder, regenerate them from scratch after substantive changes and never commit them. Embed the poster PNG that explains the core decision (up to three more only for distinct load-bearing states) and attach `deck.mp4` below it; GitHub plays an uploaded MP4 inline when its URL sits alone on a line, so prefer it over GIF. Upload by dragging the files into the PR description, or from the terminal with [`gh-img`](https://github.com/theolundqvist/gh-img) (`gh img --repo owner/repo <paths>` prints the Markdown to paste).
